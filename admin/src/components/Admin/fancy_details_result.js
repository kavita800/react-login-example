...
onTestClickFancySecond(e) {
   ...
   emptyHtml = () =>{
	...
	callFancyListApi =()=>{
		...
		// this.callMatchOddsApi()
		axios.post(baseUrl + '/getchildfancybetlist',sendObj, { headers }).then((resp) => {
				...
				//console.log(new_array[index]);
						...
						console.log(fancyNewData.matchID);
							...
							html.push(<tr role="row" className="odd">
								...
								<td className="text-left"><a href="javascript:void(0);" onClick={this.handlefancyOddsListModelOpen.bind(this,fancyNewData.event_id,fancyNewData.event_type,fancyNewData.team_name)}>{fancyNewData.headname}</a></td>
									{/*<td className="text-left"><span> {fancyNewData.lay_price}</span>&nbsp;&nbsp;&nbsp;{fancyNewData.lay_size}
								<br></br>	<br></br>
								<span> {fancyNewData.back_price}</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{fancyNewData.back_size}
									</td>*/}
								...
								<div className="action_hight">
								 <div style={{display: 'none'}} data-target={"show_data"+fancyNewData.market_id}>

								
								 <a data-target={"data_"+fancyNewData.market_id}>
								 <form id="theForm"  >
                                   <div className="row">
								   <div className="col-3">
									 <input className="form-control" type="number" min={0} required="required" Placeholder="ODDS" data-target={"data_"+fancyNewData.market_id} name={i}   ></input>
									 </div>
									 <div className="col-3">
									 <input className="form-control" type="text" required="required" Placeholder="Yes / NO"  data-target={"yes_"+fancyNewData.market_id} name={i}   ></input>
									</div>
									<div className="col-3">
									<button className="btn btn-danger33"   data-target={"button_"+fancyNewData.market_id} type="button" onClick={this.getComponent.bind(this,fancyNewData.market_id)}>Result</button>
									
									</div>
								
									
									<div className="col-12">
									<span data-target={"error_"+fancyNewData.market_id} ></span>
								 <a data-target={"data_"+fancyNewData.market_id} value={i}   className="bet-sec "></a><br></br>
								 <a data-target={"yes_no_"+fancyNewData.market_id} value={i}   className="bet-sec"></a>
								 </div>
...
{/*<a class="btn btn-default22 " onClick={this.handleclick.bind(this,fancyNewData.market_id)}href="#">Rollback</a>*/}
								 </div>
								 <div className="col-4">
									<select name="cars"  data-target={"hide_show_"+fancyNewData.market_id} onChange={this.getComponentNew.bind(this,fancyNewData.market_id)} className="form-control">
										<option value="">Please Select</option>
										<option value="suspend">Suspend</option>
										<option value="active">Active</option>
									</select>
									<span data-target={"hide_show_match"+fancyNewData.market_id}> </span>
									</div>
									</div>
</td>
			...
			if(resp.success===true){
				...
				for(var i=0;i<listData.length;i++){
					var singleListData = listData[i];
					 var htmlDataRow = <tr><td className="box-w1 lay-color fb_td"><button className="bet-sec lay"><span className="odd layprice">{singleListData.lay_price}</span>{singleListData.lay_size}</button></td><td className="box-w1 back-color fb_td"><button className="bet-sec back"><span className="odd backprice">{singleListData.back_price}</span>{singleListData.back_size}</button></td></tr>;
