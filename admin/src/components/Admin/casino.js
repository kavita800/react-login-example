...
onTestClickFancySecond(e) {
   ...
   emptyHtml = () =>{
	...
	componentWillMount() {
		...
		axios.get(baseUrl + '/api/user-casino-deatils/casino', { headers }).then((resp) => {
	...
	callMatchOddsApi=(data)=>{
			...
			const new_array = {};
				...
				var getResult1 = data.t2
				var getResult2 = data.t1
	...
	var newdata11=this.state.getResult11;
		...
		if(this.state.casinodt20!=undefined && this.state.casinodt20!='' && this.state.casinodt20!=null)
		{
			...
			for(var i=0;i<this.state.casinodt20.length;i++){
				...
				if(this.state.casinodt20[i].result==3){
					result="T"
				}
							...
							<div className="row">
								<div className="col-md-9 featured-box-detail">
									<div className="coupon-card"> 
										...
										<span className="card-header-title"> 20-20 DRAGON TIGER  <a href="#" className=" m-r-5 game-rules-icon blinking" data-id="match">RULES</a></span> 
