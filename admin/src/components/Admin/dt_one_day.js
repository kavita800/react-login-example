...
onTestClickFancySecond(e) {
   ...
   emptyHtml = () =>{
	...
	componentWillMount() {
		...
		axios.get(baseUrl + '/api/user-casino-deatils/dtoneday', { headers }).then((resp) => {
			console.log(resp.data.data);
			this.setState({casinoresult:resp.data.data})
		})
	...
	callMatchOddsApi=(data)=>{
		...
		if(data.data.t1[0]!=undefined){
			if(data.data.t1[0].autotime!=undefined){
		...
		new_array['bs1_16']=data.data.t2[16].bs1;
		new_array['l1_16']=data.data.t2[16].l1;
		new_array['ls1_16']=data.data.t2[16].ls1;
		...
		new_array['bs1_17']=data.data.t2[17].bs1;
		new_array['l1_17']=data.data.t2[17].l1;
		new_array['ls1_17']=data.data.t2[17].ls1;
		...
		new_array['bs1_18']=data.data.t2[18].bs1;
		new_array['l1_18']=data.data.t2[18].l1;
		new_array['ls1_18']=data.data.t2[18].ls1;
	...
	showTableHtml = () => {
		...
		if(this.state.casinoresult!=undefined && this.state.casinoresult!='' && this.state.casinoresult!=null){
			for (var a = 0; a < this.state.casinoresult.length; a++) {
				
				for (var b = 0; b < this.state.casinoresult[a].length;b++) {
					const value = this.state.casinoresult[a][b];
				html.push(<tr>
					<td > {value.user_id.username}  </td>
					<td > {value.team_name}  </td>
					<td > {value.odds}  </td>
					<td > {value.stake}  </td>
					<td > {Moment(value.createdDate).format('lll')}  </td>
					<td > 1 DAY DRAGON TIGER </td>
   ...
   <div className="col-md-9 featured-box-detail">
      <div className="coupon-card">
         <div className="game-heading">
            <span className="card-header-title"> 1 DAY DRAGON TIGER </span> 
...
<React.Fragment>
	{timer}
            ...
            <div className="dt-one-day_table m-b-10 main-market market-bf" data-marketid="1.167146463">
            <div className=" row">
            <div className=" col-md-6">
               <table className="coupon-table table table-bordered">
                  <thead>
					 ...
					 <tr>
					 <td><span  className="dp_block"><strong>{this.state.nat}</strong></span>
				
					 0</td>
					 <td className={b1}>
					 <div className="bet-info  "></div><button
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat,this.state.b1,'#faa9ba')}
					 
					 className="bet-sec ">
					 <span className="odd layprice " > {this.state.b1}</span>{this.state.bs1}</button></td>
					 <td  className={b1}>
					 <div className="bet-info  ">
					 </div><button
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat,this.state.l1,'#faa9ba')}
					 
					 className="bet-sec lay"><span className="odd layprice"> {this.state.l1}</span>0</button>
					 </td>
					 </tr>
					 <tr>
					 <td><span  className="dp_block"><strong>{this.state.nat_1}</strong></span>0</td>
					 <td className={b2}>
					 <div className="bet-info  "></div><button 
					  onClick={this.handleBidClick.bind(this,"lay",this.state.nat_1,this.state.b1_1,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_1}</span>0</button></td>
					 <td  className={b2}>
					 <div className="bet-info  ">
					 </div><button 
					  onClick={this.handleBidClick.bind(this,"lay",this.state.nat_1,this.state.l1_1,'#faa9ba')}
					 className="bet-sec lay"><span className="odd layprice">{this.state.l1_1}</span>{this.state.l1_1}</button>
					 </td>
					 </tr>
                  </thead>
                  <tbody>
...
<div className="col-md-6">
   <div className="d-t-box m-b-10">
            ...
            <h5 className="m-b-0 text-center text-uppercase">{this.state.b1_2}</h5>
        
        
            <b></b>
            <div className={b20}>
			<button
			onClick={this.handleBidClick.bind(this,"lay",this.state.nat_2,this.state.ls1_2,'#faa9ba')}
			className="text-uppercase btn-theme low-high-btn">
			<b>{this.state.nat_2}</b></button> 
			<span className="d-block m-t-5 text-center" >{this.state.ls1_2}</span>
			</div>
	...
	<div className=" row">		
			...
			<div className=" col-md-6">
               <table className="coupon-table table table-bordered">
					 ...
					 <tr>
					 <td><span className="dp_block"><strong>{this.state.nat_3}</strong></span>
					
					 0</td>
					 <td className={b3}>
					 <div className="bet-info  "></div><button
					 	onClick={this.handleBidClick.bind(this,"back",this.state.nat_3,this.state.b1_3,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_3}</span>0</button></td>
					<td className={b4}>
					 <div className="bet-info  "></div><button 
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_4,this.state.b1_4,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_4}</span>0</button></td>
					 </tr>
					 <tr>
					 <td><span className="dp_block"><strong>{this.state.nat_4}</strong></span>0</td>
					 <td  className={b11}>
					 <div className="bet-info  "></div><button 
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat_11,this.state.b1_11,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_11}</span>0</button></td>
					<td className={b12}>
					 <div className="bet-info  "></div><button 
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_12,this.state.b1_12,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_12}</span>0</button></td>
					 </tr>
			...
			<div className=" col-md-6">
               <table className="coupon-table table table-bordered">
                     ...
                     <tr>
                        <th> &nbsp;</th>
                        <th className="text-center  box-w1 back"> Red <span className="card-icon"><span className="card-red">{'['}</span></span><span className="card-icon"><span  className="card-red">{'{'}</span></span> </th>
                        <th className="text-center  box-w1 back" > Black <span className="card-icon"><span  className="card-black">{'}'}</span></span>
						<span className="card-icon"><span className="card-black">{']'}</span></span>
						</th>
                     </tr>
					 <tr>
					 <td><span><strong>{this.state.nat_5}</strong></span>
					 </td>
					 <td className={b5}>
					 <div className="bet-info  "></div><button
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_5,this.state.b1_5,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_5}</span>0</button></td>
					<td className={b6}>
					 <div 
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat_5,this.state.b1_6,'#faa9ba')}
					 className="bet-info  "></div><button className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_6}</span>0</button></td>
					 </tr>
					 <tr>
					 <td><span><strong>Tiger</strong></span></td>
					 <td  className={b13}>
					 <div className="bet-info  "></div><button 
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat_13,this.state.b1_13,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_13}</span>0</button></td>
					<td className={b14}>
					 <div className="bet-info  "></div><button 
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat_14,this.state.b1_14,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice">  {this.state.b1_14}</span>0</button></td>
					 </tr>
			...
			<div className=" col-md-12">
               <table className="coupon-table table table-bordered">
                     ...
                     <tr>
                        <th> &nbsp;</th>
                        <th className="text-center  box-w1 back"> <span className="card-icon"><span  className="card-black">{'}'}</span></span></th>
                        <th className="text-center  box-w1 back" > <span className="card-icon"><span  className="card-red">{'{'}</span></span></th>
                        <th className="text-center  box-w1 back" > <span className="card-icon"><span className="card-black">{']'}</span></span> </th>
                        <th className="text-center  box-w1 back" ><span className="card-icon"><span className="card-red">{'['}</span></span>  </th>
                     </tr>
					 <tr>
					 <td><span><strong>Dragon</strong></span>
					 </td>
					 <td className={b7}>
					 <div className="bet-info  "></div><button 
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat_7,this.state.b1_7,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_7}</span>0</button></td>
					<td className={b8}>
					 <div className="bet-info  "></div><button 
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_8,this.state.b1_8,'#faa9ba')}
					 
					 className="bet-sec ">
					 <span className="odd layprice">  {this.state.b1_8}</span>0</button></td>
					 <td className={b9}>
					 <div className="bet-info  "></div><button
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_9,this.state.b1_9,'#faa9ba')}
					 
					 className="bet-sec ">
					 <span className="odd layprice">  {this.state.b1_9}</span>0</button></td>
					 <td className={b10}>
					 <div className="bet-info  "></div><button
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat_10,this.state.b1_10,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice">{this.state.b1_10}</span>0</button></td>
					 </tr>
					 <tr>
					 <td><span><strong>Tiger</strong></span></td>
					 <td className={b15}>
					 <div className="bet-info  "></div><button 
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_15,this.state.b1_15,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_15}</span>0</button></td>
					<td className={b16}>
					 <div className="bet-info  "></div><button 
					  onClick={this.handleBidClick.bind(this,"back",this.state.nat_16,this.state.b1_16,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_16}</span>0</button></td>
					 <td className={b18}>
					 <div className="odd layprice"></div><button
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_18,this.state.b1_18,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_18}</span>0</button></td>
					 <td className={b17}>
					 <div className="bet-info  "></div><button
					 onClick={this.handleBidClick.bind(this,"back",this.state.nat_17,this.state.b1_17,'#faa9ba')}
					 className="bet-sec ">
					 <span className="odd layprice"> {this.state.b1_17}</span>0</button></td>
					 </tr>
   ...
   <div className="card m-b-10 place-bet ">
										...
										<a className="btn btn-primary "  href="#">Matched (0)</a>
										<a className="btn manbtn  float-right"  href="#">View More </a>
										</div>
