...
class MatchList extends Component { 
	...
	callAdminTable=()=>{
        ...
        axios.get(baseUrl + '/api/match-odds-result', {headers}).then((resp) => { 
		 ...
		 var respNew1 = resp.data1; 
			...
			if(respNew.success === true){
				...
				// this.setState({matchData:getResult,matchData2:getResult2,matchData1:respNew.inactive_matches,respStatus:respNew.success,suspend_match:respNew.suspend_match });
				// this.showTableHtml();
			...
			if(suspend_matches!=undefined){
				...
				showAction4 = 'Suspend Match';
					...
					var mainHtml=""
							...
							<td className="text-right"><a href={"/fancy-details/"+value.match_id+"/"+this.props.match.params.id}>{value.match_name+"/"+Moment(value.openDate).format('lll')}</a></td>
	  ...
	  <Modal show={this.state.showWinModel} onHide={this.handleInactiveModelClose}>
		<form className="form loginform" onSubmit={this.handleBetLimitModelSubmit}>
			...
			<div className="row form-group">
			  ...
			  <div className="col-md-8">
				<input type="text" required name="max_bet_limit" onChange={this.handleChangeData}  className="form-control intr" />
			  </div>
			  </div>
			  <div className="row form-group">
			  <div className="col-md-4">
			  ...
			  <div className="col-md-8">
				<input type="text" required name="min_bet_limit" onChange={this.handleChangeDataNew}  className="form-control intr" />
			  </div>
			  </div>
