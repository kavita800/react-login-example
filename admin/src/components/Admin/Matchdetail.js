...
class Index extends Component {
	...
	betDeleteClickConfirm=(betId,betStatus)=>{
		
		confirmAlert({
		  title: 'Confirm',
		  message: 'Are you sure to convert this bet to '+betStatus,
		  buttons: [
			{
			  label: 'Yes',
			  onClick: () => this.callDeleteBet(betId,betStatus)
			},
			{
			  label: 'No',
	...
	callDeleteBet=(betId,betStatus)=>{
		...
		axios.post(baseUrl + '/api/updatebetstatus',sendBetObj,{headers}).then((resp) => {  
			var resp = resp.data;
			
			
			this.setState({betUpdateResp:resp.success,betUpdateMsg:resp.message});
			this.callUserBetListApi();
			setTimeout(() => {this.setState({ betUpdateResp: "",betUpdateMsg:""});}, 6000);
		});  
	}
	
	showBetUpdateRespHtml=()=>{
		if(this.state.betUpdateResp===true){
			return (
		<div className="alert alert-success">{this.state.betUpdateMsg}</div>
			)
		}
		else if(this.state.betUpdateResp===false){
			return (
				<div className="alert alert-danger">{this.state.betUpdateMsg}</div>
			)
		}
	}
   ...
   emptyHtml = () =>{
		...
		if (this.state.fancyDataFound === true) {
			...
			if(this.state.getResults!=undefined){
				for(var i=0;i<this.state.getResults.length;i++){
					if(this.state.getResults.bet_type=="fancy"){
						if(this.state.getResults.key_index!=null && this.state.getResults.key_index!=undefined){
							new_array[this.state.getResults.key_index]=(this.state.getResults.key_index);
							new_array1[this.state.getResults.key_index]=(this.state.getResults.stake);
							new_array2[this.state.getResults.key_index]=(this.state.getResults._id);
						}
							...
							if(fancyNewData.SessInptNo !==undefined && fancyNewData.SessInptYes!==undefined){
								...
								//if(fancyNewData.DisplayMsg!="SUSPENDED"){
									html.push(<tr>
										...
										<td className="fb_td" style={{ textAlign: "center" }}> Min 10 <br/> Max 10000  </td>
									</tr>);
								//}
							}
						 }else{
	...
	callUserBetListApi=()=>{
		...
		let matchid = this.props.match.params.id;
		axios.get(baseUrl + '/api/getuserbetmatchesid/'+matchid, { headers }).then((resp) => {
			var resps = resp.data;
			var profit12=0;
			var profit13=0;
		
				 this.setState({ getResults: resps.data, betDataFound: true });
			
		});
	
		axios.get(baseUrl + '/api/getuserbetmatches1/'+matchid, { headers }).then((resp) => {
			var resps = resp.data;
			if(resps.data[0]!=undefined){
				this.setState({first_team_data:resps.data[0].firstTeamTotal,second_team_data:resps.data[0].secondTeamTotal,
				drawTeamTotal:resps.data[0].drawTeamTotal
				})
			}
			//this.setState({oddsdata:resps.data[0]})
			
		});
	
		axios.get(baseUrl + '/api/getuserbetmatchesbookmaker/'+matchid, { headers }).then((resp) => {
			var resps = resp.data;
			if(resps.data[0]!=undefined){
				this.setState({first_team_data1:resps.data[0].firstTeamTotal,second_team_data1:resps.data[0].secondTeamTotal,
				drawTeamTota1:resps.data[0].drawTeamTotal
				})
			}
			//this.setState({oddsdata:resps.data[0]})
		...
		axios.get(baseUrl + '/api/getuserbetmatchesfancy/'+matchid, { headers }).then((resp) => {
			var resps = resp.data;
			//console.log(resps.data);
			this.setState({data123:resps.data})
			...
			return (
				<tr className="bet-info ">
					<td className="team-name nation" >
						<span ><strong>The Draw</strong></span>
						<p className="box-w4"><span className="float-left book" id="book_349" style={{ color: "black" }}>{this.state.drawTeamTotal}</span> <span className="float-right profit" id="profit_349" style={{ color: "black" }} >
					...
					<td className="box-w1 back-color" style={{backgroundColor:'#B2D6F0'}}>
						{blockHtml}
						{status}
						<button className="bet-sec back "  onClick={this.handleBidClick.bind(this,this.state.drawTeamName,this.state.drawLastBack,'#B2D6F0',"back",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'odds')}> <span className="odd backprice">{this.state.drawLastBack}</span>{this.state.drawLastBackSize}  </button>
					</td>
					<td className="box-w1 back-color" style={{backgroundColor:'#92C9F0'}}>
						<button className="bet-sec back " onClick={this.handleBidClick.bind(this,this.state.drawTeamName,this.state.drawMiddleBack,'#92C9F0',"back",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'odds')} > <span className="odd backprice">{this.state.drawMiddleBack}</span> {this.state.drawMiddleBackSize} </button>
					</td>
					
					<td className="box-w1 back-color">{status}
						<button className="bet-sec back " onClick={this.handleBidClick.bind(this,this.state.drawTeamName,this.state.drawFirstBack,'#72bbef',"back",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'odds')} > <span className="odd backprice">{this.state.drawFirstBack}</span> {this.state.drawFirstBackSize} </button>
					</td>
					
					
					
					<td className="box-w1 lay-color">{status}
						<button className="bet-sec lay" onClick={this.handleBidClick.bind(this,this.state.drawTeamName,this.state.drawFirstLay,'#faa9ba',"lay",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'odds')} value={this.state.lastPriceTraded1} ><span className="odd layprice">{this.state.drawFirstLay}</span>{this.state.drawFirstLaySize}</button>
					</td>
					<td className="box-w1 lay-color" style={{backgroundColor:'#F8BBC8'}}>{status}
						<button className="bet-sec lay" value={this.state.lastPriceTraded1} onClick={this.handleBidClick.bind(this,this.state.drawTeamName,this.state.drawMiddleLay,'#F8BBC8',"lay",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'odds')} ><span className="odd layprice">{this.state.drawMiddleLay}</span>{this.state.drawMiddleLaySize}</button>
					</td>
					<td className="box-w1 lay-color" style={{backgroundColor:'#F6CDD6'}}>{status}
						<button className="bet-sec lay" value={this.state.lastPriceTraded1} onClick={this.handleBidClick.bind(this,this.state.drawTeamName,this.state.drawLastLay,'#F6CDD6',"lay",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'odds')}><span className="odd layprice">{this.state.drawLastLay}</span>{this.state.drawLastLaySize}</button>
					</td>															
...
var suspended =(this.state.bookmaker_a_back_1==0) ? "bet-info suspended row" : "bet-info row";
			
			returnHtml.push(
			<div className={suspended}>
						<div className="team-name nation col-6" id="10301">
							<span><strong>{this.state.firstTeamName}</strong></span>
							<p className="box-w4"><span className="float-right book" id="book_10301" style={{ color: "black" }}>  </span> <span className="float-right profit" id="profit_10301" style={{ color: "black" }}>{this.state.team_profit4}
																 {this.state.stackAmount_team4}</span>
																 </p>
							<p className="box-w4"><span className="float-left book"  style={{ color: "black" }}> {this.state.first_team_data1}
							</span> <span className="float-left  profit" style={{ color: "black" }} >
								
								</span></p>
						</div>
						<div className="box-w1 back-color col-1" style={{backgroundColor:'#B2D6F0'}}>{status}
							<button className="bet-sec back "  onClick={this.handleBidClickBook.bind(this,this.state.firstTeamName,this.state.bookmaker_a_back_3,'#B2D6F0',"back",this.state.teamOneSelectionId,this.state.marketId,'teamone',this.state.status,'bookmaker')}> <span className="odd backprice" >{this.state.bookmaker_a_back_3}</span>{this.state.bookmaker_a_back_6}</button>
						</div>
						<div className="box-w1 back-color col-1" style={{backgroundColor:'#92C9F0'}}>{status}
						...
						<div className="box-w1 lay-color col-1">{status}
							<button className="bet-sec lay"  onClick={this.handleBidClickBook.bind(this,this.state.firstTeamName,this.state.bookmaker_a_lay_1,'#FAA9BA',"lay",this.state.teamOneSelectionId,this.state.marketId,'teamone',this.state.status,'bookmaker')}><span className="odd layprice">{this.state.bookmaker_a_lay_1}</span>{this.state.bookmaker_a_lay_4}</button>
						</div>
						<div className="box-w1 lay-color col-1" style={{backgroundColor:'#F8BBC8'}}>{status}
							<button className="bet-sec lay"  onClick={this.handleBidClickBook.bind(this,this.state.firstTeamName,this.state.bookmaker_a_lay_2,'#F8BBC8',"lay",this.state.teamOneSelectionId,this.state.marketId,'teamone',this.state.status,'bookmaker')}><span className="odd layprice">{this.state.bookmaker_a_lay_2 }</span>{this.state.bookmaker_a_lay_5}</button>
						</div>
						<div className="box-w1 lay-color col-1" style={{backgroundColor:'#F6CDD6'}}>{status}
							<button className="bet-sec lay"  onClick={this.handleBidClickBook.bind(this,this.state.firstTeamName,this.state.bookmaker_a_lay_3,'#F6CDD6',"lay",this.state.teamOneSelectionId,this.state.marketId,'teamone',this.state.status,'bookmaker')}><span className="odd layprice">{this.state.bookmaker_a_lay_3}</span>{this.state.bookmaker_a_lay_6}</button>
						</div>	
						</div>);
		}
		
		if(this.state.bookmakerSecondTeamFound===true) {
			var suspended1 =(this.state.bookmaker_b_back_1==0) ? "bet-info suspended row" : "bet-info row";
			...
			returnHtml.push(<div className={suspended1}>
					...
					{status}
						...
						<button className="bet-sec back "  onClick={this.handleBidClickBook.bind(this,this.state.secondTeamName,this.state.bookmaker_b_back_3,'#B2D6F0',"back",this.state.teamTwoSelectionId,this.state.marketId,'teamone',this.state.status,'bookmaker')}  > <span className="odd backprice">{this.state.bookmaker_b_back_3} </span>{this.state.bookmaker_b_back_6} </button>
					</div>
					<div className="box-w1 back-color col-1" style={{backgroundColor:'#92C9F0'}}>{status}
						<button className="bet-sec back " onClick={this.handleBidClickBook.bind(this,this.state.secondTeamName,this.state.bookmaker_b_back_2,'#92C9F0',"back",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,'bookmaker')} > <span className="odd backprice">{this.state.bookmaker_b_back_2}</span> {this.state.bookmaker_b_back_5} </button>
					</div>
					
					<div className="box-w1 back-color col-1">{status}
						<button className="bet-sec back " onClick={this.handleBidClickBook.bind(this,this.state.secondTeamName,this.state.bookmaker_b_back_1,'#92C9F0',"back",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,'bookmaker')} > <span className="odd backprice">{this.state.bookmaker_b_back_1}</span> {this.state.bookmaker_b_back_4} </button>
					</div>
					
					
					
					<div className="box-w1 lay-color col-1">{status}
						<button className="bet-sec lay"  onClick={this.handleBidClickBook.bind(this,this.state.secondTeamName,this.state.bookmaker_b_lay_1,'#faa9ba',"lay",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,'bookmaker')}><span className="odd layprice">{this.state.bookmaker_b_lay_1}</span>{this.state.bookmaker_b_lay_4}</button>
					</div>
					<div className="box-w1 lay-color col-1" style={{backgroundColor:'#F8BBC8'}}>{status}
						<button className="bet-sec lay"  onClick={this.handleBidClickBook.bind(this,this.state.secondTeamName,this.state.bookmaker_b_lay_2,'#F8BBC8',"lay",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,'bookmaker')}><span className="odd layprice">{this.state.bookmaker_b_lay_2}</span>{this.state.bookmaker_b_lay_5}</button>
					</div>
					<div className="box-w1 lay-color col-1" style={{backgroundColor:'#F6CDD6'}}>{status}
						<button className="bet-sec lay"  onClick={this.handleBidClickBook.bind(this,this.state.secondTeamName,this.state.bookmaker_b_lay_3,'#F6CDD6',"lay",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,'bookmaker')}><span className="odd layprice">{this.state.bookmaker_b_lay_3}</span>{this.state.bookmaker_b_lay_6}</button>
					</div>															
			...
			if(this.state.bookmaker_draw===true) {
				var suspended2 =(this.state.bookmaker_d_back_1==0) ? "bet-info suspended row" : "bet-info row";
			
				returnHtml.push(<div className={suspended2}>
					<div className="team-name nation col-6" >
						<span ><strong>Draw</strong></span>
						<p className="box-w4"><span className="float-left book" id="book_349" style={{ color: "black" }}>{this.state.drawTeamTotal1}</span> <span className="float-right profit" id="profit_349" style={{ color: "black" }} >
					...
					<div className="box-w1 back-color col-1" style={{backgroundColor:'#B2D6F0'}}>{status}
						<button className="bet-sec back "  onClick={this.handleBidClickBook.bind(this,this.state.drawTeamName,this.state.bookmaker_d_back_3,'#B2D6F0',"back",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'bookmaker')}> <span className="odd backprice">{this.state.bookmaker_d_back_3}</span>{this.state.bookmaker_d_back_6}  </button>
					</div>
					<div className="box-w1 back-color col-1" style={{backgroundColor:'#92C9F0'}}>
						<button className="bet-sec back " onClick={this.handleBidClickBook.bind(this,this.state.drawTeamName,this.state.bookmaker_d_back_2,'#92C9F0',"back",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'bookmaker')} > <span className="odd backprice">{this.state.bookmaker_d_back_2}</span> {this.state.bookmaker_d_back_5} </button>
					</div>
					
					<div className="box-w1 back-color col-1">{status}
						<button className="bet-sec back " onClick={this.handleBidClickBook.bind(this,this.state.drawTeamName,this.state.bookmaker_d_back_1,'#72bbef',"back",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'bookmaker')} > <span className="odd backprice">{this.state.bookmaker_d_back_1}</span> {this.state.bookmaker_d_back_4} </button>
					</div>
					
					
					
					<div className="box-w1 lay-color col-1">{status}
						<button className="bet-sec lay" onClick={this.handleBidClickBook.bind(this,this.state.drawTeamName,this.state.bookmaker_d_lay_1,'#faa9ba',"lay",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'bookmaker')} value={this.state.bookmaker_d_lay_4} ><span className="odd layprice">{this.state.bookmaker_d_lay_1}</span>{this.state.bookmaker_d_lay_4}</button>
					</div>
					<div className="box-w1 lay-color col-1" style={{backgroundColor:'#F8BBC8'}}>{status}
						<button className="bet-sec lay" value={this.state.bookmaker_d_lay_5} onClick={this.handleBidClickBook.bind(this,this.state.drawTeamName,this.state.bookmaker_d_lay_2,'#F8BBC8',"lay",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'bookmaker')} ><span className="odd layprice">{this.state.bookmaker_d_lay_2}</span>{this.state.bookmaker_d_lay_5}</button>
					</div>
					<div className="box-w1 lay-color col-1" style={{backgroundColor:'#F6CDD6'}}>{status}
						<button className="bet-sec lay" value={this.state.bookmaker_d_lay_6} onClick={this.handleBidClickBook.bind(this,this.state.drawTeamName,this.state.bookmaker_d_lay_3,'#F6CDD6',"lay",this.state.drawSelectionId,this.state.marketId,'draw',this.state.status,'bookmaker')}><span className="odd layprice">{this.state.bookmaker_d_lay_3}</span>{this.state.bookmaker_d_lay_6}</button>
					</div>															
	...
	showTableHtml = () => {
		...
		if (this.state.betDataFound === true) {
			...
			for(let i=0;i<this.state.getResults.length;i++){
				...
				var setNewStatus = (value.status==="pending") ? "deleted" : "pending";
				var setActionIcon = (value.status==="pending") ? "fa-trash" : "fa-undo";
				var setActionIconColor = (value.status==="pending") ? "red" : "green";
				var ActionBtn="";
				if(value.status!=="completed") {
					 ActionBtn= (this.state.userData.userType===1) ? <i onClick={this.betDeleteClickConfirm.bind(this,value._id,setNewStatus)} style={{color:setActionIconColor}} className={"fa "+setActionIcon}></i> : "";
				}
				if(value.type==="match"){
				...
				html.push(<tr style={{background: colorClass }} >
					...
					<td style={{ textAlign: "center" }}> {ActionBtn} </td>
				</tr>);
				}else{
					html111.push(<tr style={{background: colorClass }} >
						<td  style={{ textAlign: "center",}}> {fullname}  </td>
						<td  style={{ textAlign: "center",}}> {value.team_name}  </td>
						<td style={{ textAlign: "center" }}> {value.odds}  </td>
						<td style={{ textAlign: "center" }}> {value.stake}  </td>
			...
			<div className="tab-content">
			  ...
			  <div id="menu1" className="container tab-pane "><br />
			  <table className="table coupon-table">
					<thead>
					<tr>
						<th style={{ textAlign: "center" }} > User Name</th>
	...
	handleChange = (event) => {
		...
		if(this.state.betClick==true){
			if(this.state.type === "back") {
				...
				//profit = parseFloat(this.state.oddVal)-1;
					//profit = getAmount.toFixed(2) * getAmount;
					profit = parseFloat(this.state.oddVal)-1;
					profit = profit.toFixed(2) * event.target.value;
				if(this.state.betMatchType=="teamone"){
					this.setState({team_profit1:"-"+profit})
		...
		if(this.state.betClick2==true){
			if(this.state.type === "back") {
				...
				profit = profit.toFixed(2) *parseFloat(event.target.value);
				 ...
				 // ////////////console.logthis.state.betMatchType);
				if(this.state.betMatchType=="teamone"){
					this.setState({team_profit4:profit})
					
				}
				 if(this.state.betMatchType=="teamtwo"){
					this.setState({team_profit5:profit})
			
			
				}
				if(this.state.betMatchType=="draw"){
					this.setState({team_profit6:profit})
				}
	...
	handleBidClick = (teamName,oddVal,color,type,selectionId,marketId,getMatchType,status,betOn) =>{
	...
	handleBidClickBook = (teamName,oddVal,color,type,selectionId,marketId,getMatchType,status,betOn) =>{
	...
	handleBidClickSession = (color,headname,no,marketId,SessInptNo,yes,index1,yes_no_STAKE) =>{
			if(SessInptNo!=0){
	...
	showBidClickHtml = () =>{
		...
		// 					<td>
		// 						<div className="form-group bet-stake">
		// 							<input id="btn_val"  ref={this.emailInput} style={{ width: "52px" }} maxLength="10" value={this.state.stake_amount} name="stake_amount" type="text" onChange={this.handleChange} required="required" />
		// 						</div>
		// 					</td> 
		// 					<td>
		// 					{this.state.profit}
	...
	handleBetLockClick=(status,type)=>{
		...
		if(status==1 || status==2){
			...
			axios.get(baseUrl + '/api/betlock-history/'+this.props.match.params.id+"/"+status+"/"+"odds",{headers}).then((resp) => {  
				var resp= resp.data;
				this.setState({maxminBet:resp})
			}); 
			if(status==1) {
				alert("Bet Lock Successfully");
			}
			if(status==2) {
				alert("Bet UnLock Successfully");
			}
		}else{
			let headers={
				Authorization:"Bearer "+ this.state.accessToken,
			}; 
			 axios.get(baseUrl + '/api/betlock-history/'+this.props.match.params.id+"/"+type, {headers}).then((resp) => {
				var html =[];
			...
			this.setState({betlockHistory:true})
		}
		
		
		//this.setState({respStatus:true,respMessage:"Status Updated Successfully."});
	}
	handleBetLockClick1=(status,type)=>{
		...
		if(status==1 || status==2){
			...
			axios.get(baseUrl + '/api/betlockuser/'+this.props.match.params.id+"/"+status+"/"+type,{headers}).then((resp) => {  
				var resp= resp.data;
				this.setState({maxminBet:resp})
			}); 
			if(status==1) {
	...
	showTableHtmlBetLock = () =>{ 
		if(this.state.bethistory!=undefined){  
		   var html1 = []  
		  	...
		  	for (var a = 0; a < this.state.bethistory.length; a++) {
				
				for (var b = 0; b < this.state.bethistory[a].length;b++) {
					const value = this.state.bethistory[a][b];
					//console.log(value);
					var check="";
					if(value.status==1){
						check ="Bet Lock"
				 ...
				 var mainHtml = <tr role="row" className="odd" >
					   ...
					   <td>{value.user_id.username} &nbsp;&nbsp;&nbsp;&nbsp;
					  </td>
					  <td> <span id={"check_11_"+a}>{check}</span>
					 
					  <div id={"show_message_"+a}></div>
					  </td>
					  <td>{value.type}</td>
					   <td><input type="checkbox"   onClick={this.handleBidClickChekbox.bind(this,value._id,a)}  >
	...
	// 	if (this.state.betDataFound1 === true) {
	// 		//console.log(this.state.userdataFoundNew);
	// 		const html = []
	// 		var profit12=0;
	// 		var profit13= 0;
	// 		var loss=0;
	// 		var cricket_partnership_own =0;
	// 		//var custom_this=this;
	// 		var teamOneSelectionId=this.state.teamOneSelectionId;
	// 		var teamTwoSelectionId=this.state.teamOneSelectionId;
	// 		let headers={
	...
	// 		var arr_data=[];
	// 		var c =0;
	// 		for(let i=0;i<this.state.getResults1.length;i++){
	// 			////console.log(value);
	// 			for (var b = 0; b < this.state.getResults1[i].length;b++) {
	// 				const value = this.state.getResults1[i][b];
	// 				//console.log(value);
	// 				// if(index==1)
	...
	// 			if(value.bet_type!="fancy" && value.bet_type!="bookmaker"){
	// 				if(this.state.userData!=undefined){

					
	// 					var cricket_partnership_own =this.state.userData.cricket_partnership_own;
	// 					if(this.state.userData.cricket_partnership_own==0){
	// 						var cricket_partnership_own =100;
	// 					}
						
	// 					profit12 =value.profit*cricket_partnership_own/100;
						
	// 					if(Math.sign(profit12)===1){
	// 						profit12 = "-"+profit12;
	// 					}else{
	// 						profit12 = Math.abs(profit12);
	// 					} 
	// 					}if(value.bet_type=="fancy"){
	// 						profit12 = "-"+value.stake;
	// 					}

						
	// 					//  result[value.user_id.username] = result[value.user_id.username] || {profit12:0}
	// 					//  result[value.user_id.username] += profit12 ;
					
	// 					if (result[value.user_id.username]) { //(4)
	// 						result[value.user_id.username] +=parseFloat(profit12);
	// 						// arr_data[c]['username']=value.user_id.username;
	// 						// arr_data[c]['total'] +=(profit12);
	// 						//(5)
	// 					  } else { //(6)
	...
	// 			var result2= Object.keys(result);
	// 			for(var data=0;data<result1.length;data++){
	// 				//console.log(result2[data]);
	// 				html.push(<tr >
	// 					<td  style={{ textAlign: "center",}}> {result2[data]}   </td>
	// 					<td style={{ textAlign: "center" }}>{result1[data]}  </td>
	...
	// 				// axios.get(baseUrl + '/current_user_data/'+value.user_id.parentid,{headers}).then(data)
	// 				// 	cons
	// 				// }
	...
	// 	}   
		handleBetLockClick=(status,type)=>{
			...
			if(status==1 || status==2){
				...
				axios.get(baseUrl + '/api/betlockuser/'+this.props.match.params.id+"/"+status+"/"+"odds",{headers}).then((resp) => {  
					var resp= resp.data;
					this.setState({maxminBet:resp})
				}); 
				if(status==1) {
		...
		handleUserLockClick=()=>{
			...
			let headers = {
			 ...
			 let matchid = this.props.match.params.id;
			// this.callMatchOddsApi()
			axios.get(baseUrl + '/api/userbetlist_admin/'+matchid, { headers }).then((resp) => {
				var resps = resp.data;
				var profit12=0;
				var profit13=0;
				if (resps.success === true) {
					this.setState({ getResults1: resps.data, betDataFound1: true });
				}
			});
			
			
				
				this.setState({userLogHistory:true})
		...
		handleUserBookClick=()=>{
			...
			let headers = {
			 ...
			 let matchid = this.props.match.params.id;
			// this.callMatchOddsApi()
			axios.get(baseUrl + '/api/userbetlistbook_admin/'+matchid, { headers }).then((resp) => {
				var resps = resp.data;
				var profit12=0;
				var profit13=0;
				if (resps.success === true) {
		...
		betlockHistoryHide=()=>{
			...
			//this.setState({respStatus:true,respMessage:"Status Updated Successfully."});
			this.setState({userLogHistory:false})
		}
		...
		handleBidClickChekbox = (id,a) => {
			
			let headers={
				Authorization:"Bearer "+ this.state.accessToken,
			}; 
			axios.get(baseUrl + '/api/betlock-update/'+id, {headers}).then((resp) => {
				alert(resp.data.message);
				$('#check_11_'+a).remove();
				$('#show_message_'+a).html(resp.data.message);
		...
		showTableHtmlBetLock = () =>{ 
		   ...
		   showTableUserLock = () =>{ 
			...
			var value=this.state.getResults1[i];
					// 	////console.log(value);
				// 		//console.log(value);
				// 		// if(index==1)
				...
				// 		////console.log(element);
							...
							//  result[value.username] = result[value.username] || {firstTeamTotal:0}
							//  result[value.username] += value.firstTeamTotal ;
						console.log();
				...
				// 		//console.log(result2[data]);
						html.push(<tr >
							...
							<td  style={{ textAlign: "center",}}> {value.firstTeamTotal}   </td>
							<td style={{ textAlign: "center" }}>{value.secondTeamTotal}  </td>
							<td style={{ textAlign: "center" }}>{value.drawTeamTotal}  </td>
			...
			betlockHistoryHide2 = () => {
				...
				this.setState({betlockHistory:false})
			}
	...
	render() {
		...
		var new_array=[];
		var new_array1="";
		var datafancy=this.state.data123;
		if(this.state.sessiondata!=undefined){
			for(var i=0;i<this.state.sessiondata.length;i++){
				...
				// if(data123[i].teamname==this.state.sessiondata[i].RunnerName){
					var newteamname=this.state.sessiondata[i].RunnerName;
					var toatlprice="";
					var found = data123.findIndex(function (element) { 
					
						return element.teamname === newteamname; 
					}); 
...
//}
				if(this.state.sessiondata[i].LayPrice1==0){
					fancy_suspend ="box-w1 lay-color fb_td suspended ";
				}
				if(this.state.sessiondata[i].BackPrice1==0){
				...
				{this.handleBidClickSession.bind(this, '#faa9ba',this.state.sessiondata[i].RunnerName,"no",this.state.sessiondata[i].marketId,this.state.sessiondata[i].LayPrice1,"",this.state.sessiondata[i].market_id)}
				...
				<button className="bet-sec back" value={i} 
				 onClick= 
				 {this.handleBidClickSession.bind(this, '#72bbef',this.state.sessiondata[i].RunnerName,"no",this.state.sessiondata[i].marketId,this.state.sessiondata[i].BackPrice1,"",this.state.sessiondata[i].market_id)}
...
if(this.state.no_data_model!=undefined){
		...
		if(this.state.no_data_model.data.showdata!=undefined)
	{
		...
		var new_array12333=<div>
			<div className="row form-group">
			  <div className="col-md-4">
				<label></label>
			  </div>
			  <div className="col-md-8">Amount
			  </div>
			  </div>
			<div className="row form-group">
			  <div className="col-md-4" 
	...
	new_fancy_arr.push(new_array12333);
	}
	}
						...
						<div className="container-fluid">
							<div className="row">
								<div className="col-md-9 featured-box-detail">
									...
									<div className="game-heading"> <span className="card-header-title">{this.state.firstTeamName} v {this.state.secondTeamName}</span>
									<span className="card-header-title" style={{float:'right'}}>{Moment(this.state.openMatchDate).format('lll')}</span> 
										 </div> <br></br>
													...
													<tbody id="dyn_bind">
														<tr className="bet-info ">
															<td className="team-name nation" >
																...
																<p className="box-w4"><span className="float-right book" id="book_349" style={{ color: "black" }}>{this.state.team_profit1}
																 ...
																 <p className="box-w4"><span className="float-left book"  style={{ color: "black" }}>{this.state.first_team_data}
																</span> <span className="float-left  profit" style={{ color: "black" }} ></span></p>
														...
														className="bet-info ">
															<td className="team-name nation" id="10301">
																...
																<p className="box-w4"><span className="float-left book"  style={{ color: "black" }}>{this.state.second_team_data}
																</span> <span className="float-left  profit" style={{ color: "black" }} ></span></p>
															</td>
															<td
																 id="blockin7"
															className="box-w1 back-color" style={{backgroundColor:'#B2D6F0'}}>{blockOddsHtml}{status}
																<button className="bet-sec back " onClick={this.handleBidClick.bind(this,this.state.secondTeamName,this.state.teamTwoLastBack,'#B2D6F0',"back",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,"odds")} > <span className="odd backprice" >{this.state.teamTwoLastBack}</span>{this.state.teamTwoLastBackSize}</button>
															</td>
															<td
															id="blockin8"
															className="box-w1 back-color" style={{backgroundColor:'#92C9F0'}}>{status}
																<button className="bet-sec back "  onClick={this.handleBidClick.bind(this,this.state.secondTeamName,this.state.teamTwoMiddleBack,'#92C9F0',"back",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,"odds")}> <span className="odd backprice" >{this.state.teamTwoMiddleBack}</span>{this.state.teamTwoMiddleBackSize}  </button>
															</td>
															
															<td 
															id="blockin9"
															className="box-w1 back-color">{status}
																<button className="bet-sec back "  onClick={this.handleBidClick.bind(this,this.state.secondTeamName,this.state.teamTwoFirstBack,'#72bbef',"back",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status)}> <span className="odd backprice" >{this.state.teamTwoFirstBack}</span>{this.state.teamTwoFirstBackSize}  </button>
															</td>
															...
															id="blockin10"
															className="box-w1 lay-color">{status}
																<button className="bet-sec lay" value={this.state.lastPriceTraded1} onClick={this.handleBidClick.bind(this,this.state.secondTeamName,this.state.teamTwoFirstLay,'#FAA9BA',"lay",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,"odds")}><span className="odd layprice">{this.state.teamTwoFirstLay}</span>{this.state.teamTwoFirstLaySize}</button>
															</td>
															<td 
															id="blockin11"
															className="box-w1 lay-color" style={{backgroundColor:'#F8BBC8'}}>{status}
																<button className="bet-sec lay" value={this.state.lastPriceTraded1} onClick={this.handleBidClick.bind(this,this.state.secondTeamName,this.state.teamTwoMiddleLay,'#F8BBC8',"lay",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,"odds")}><span className="odd layprice">{this.state.teamTwoMiddleLay}</span>{this.state.teamTwoMiddleLaySize}</button>
															</td>
															<td 
															id="blockin12"
															className="box-w1 lay-color" style={{backgroundColor:'#F6CDD6'}}>{status}
																<button className="bet-sec lay" value={this.state.lastPriceTraded1} onClick={this.handleBidClick.bind(this,this.state.secondTeamName,this.state.teamTwoLastLay,'#F6CDD6',"lay",this.state.teamTwoSelectionId,this.state.marketId,'teamtwo',this.state.status,"odds")}><span className="odd layprice">{this.state.teamTwoLastLay}</span>{this.state.teamTwoLastLaySize}</button>
															</td>	
														</tr>
														{this.showDrawHtml()}
													</tbody>
...
<div className="row row-12 ssw_66">
	<div className="col-md-12" id="fancyHeadToHide" >
		<div class="game-heading"> <span class="card-header-title">Bookmaker Market <a href="#" class=" m-r-5 game-rules-icon blinking" data-id="match"><span><i class="fa fa-info-circle float-right"></i></span></a></span><span class="float-right m-r-10">Maximum Bet :  <span id="maxbetlmt">{this.state.max_bookmaker_limit}</span></span><span class="float-right m-r-10">Minimum Bet :<span id="maxbetlmt">{this.state.min_bookmaker_limit}</span></span></div>
		<div id="div_fancy">
		<div className="card-content">
											<div className="table-responsives  m-b-10 main-market market-bf" data-marketid="1.167146463">
												<div className="table new_table  coupon-table table table-striped table-bordered m-t-10 " >
	...
	<div className="col-md-6 fancy-market">
								...
								<ul class="dropdown-menu">
									<li><a onClick={this.handleBetLockClick.bind(this,1,"odds")} href="#">Lock</a></li>
									<li><a  onClick={this.handleBetLockClick.bind(this,2,"odds")} href="#">Unlock</a></li>
									<li><a onClick={this.handleBetLockClick.bind(this,3,"odds")} href="#">Select user</a></li>
        						</ul>
  ...
  <li><a onClick={this.handleBetLockClick1.bind(this,1,"fancy")} href="#">Lock</a></li>
									<li><a  onClick={this.handleBetLockClick1.bind(this,2,"fancy")} href="#">Unlock</a></li>
									<li><a onClick={this.handleBetLockClick1.bind(this,3,"fancy")} href="#">Select user</a></li>
  </div>
</div>
</div>
<div className="col-33">
								<button onClick={this.handleUserLockClick}  class="btn btn-primary">User Book</button>
								
								</div>

								<div className="col-33">
								<button onClick={this.handleUserBookClick}  class="btn btn-primary">Bookmaker Book</button>
  ...
  <li><a onClick={this.handleBetLockClick1.bind(this,1,"bookmaker")} href="#">Lock</a></li>
									<li><a  onClick={this.handleBetLockClick1.bind(this,2,"bookmaker")} href="#">Unlock</a></li>
									<li><a onClick={this.handleBetLockClick1.bind(this,3,"bookmaker")} href="#">Select user</a></li>
  </div>
</div>
</div>
</div>
									...
									<div className="card m-b-10 mt-3 place-bet ">
					...
					<Modal className="admin-trns" show={this.state.betlockHistory} onHide={this.betlockHistoryHide2}>
			<a class="close" data-dismiss="modal" onClick={this.betlockHistoryHide2} >×</a>
			<Modal.Header >
	  ...
	  <Modal className="admin-trns" show={this.state.userLogHistory} onHide={this.userLogkHistoryHide}>
			<a class="close" data-dismiss="modal" onClick={this.userLogHistoryHide} >×</a>
			<Modal.Header >
				<Modal.Title></Modal.Title>
			</Modal.Header>
			<Modal.Body>
				 ...
				 <div className="table-responsive">
					<table id="account-statement" className="display dataTable" >
					   <thead>
						  ...
						  <th >User Name</th>
							 <th >{this.state.firstTeamName}</th>
							 <th >{this.state.secondTeamName}</th>
							 <th >Draw Team Total</th>
						  </tr>
					   </thead>

					  {this.showTableUserLock()}
					</table>
				 </div>
				...
				<div id="content-wrapper">
					<div className="container-fluid">
						<div className="row">
							<div className="col-md-9 featured-box-detail">
								<div className="coupon-card"> 
									<div className="game-heading"> <span className="card-header-title">MATCH ODDS <a href="#" className=" m-r-5 game-rules-icon blinking" data-id="match"><span><i className="fa fa-info-circle float-right"></i></span></a></span> <span className="float-right m-r-10"> <span id="maxbetlmt">{this.state.userCurretBalance}</span></span> </div>
									<div className="card-content">
										<div className="table-responsive m-b-10 main-market market-bf" data-marketid="1.167146463">
											<table className="table coupon-table table table-striped table-bordered m-t-10 " >
												<thead>
