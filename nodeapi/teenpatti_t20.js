...
io.on("connection", (socket) => {
 // if (interval) {
  ...
  //}
  interval = setInterval(() => getApiAndEmit8(socket), 1000);
 ...
 const getApiAndEmit8 = async socket => {
...
var axios = require('axios');
	...
	var config = {
	  method: 'get',
	  url: 'http://172.105.49.149:8000/lordsexch/teen20/',
	  headers: { 
	...
	var getData = await  axios(config);
	  ...
	  socket.emit("FromAPI8",{data:getData.data.data,data1:getData.data.result});
