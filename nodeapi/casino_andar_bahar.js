...
io.on("connection", (socket) => {
 ...
 // }
  interval = setInterval(() => getApiAndEmit10(socket), 1000);
 ...
 const getApiAndEmit10= async socket => {
...
var axios = require('axios');
	...
	var config = {
	  method: 'get',
	  url: 'http://172.105.53.226:3000/bmwexch/andarbahar',
	  headers: { 
	...
	var getData = await  axios(config);
	  ...
	  socket.emit("FromAPI10",{data:getData.data.data,data1:getData.data.result});
