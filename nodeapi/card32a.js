...
io.on("connection", (socket) => {
  ...
  //}
  interval = setInterval(() => getApiAndEmit9(socket), 1000);
 ...
 const getApiAndEmit9 = async socket => {
...
var axios = require('axios');
	...
	var config = {
	  method: 'get',
	  url: 'http://172.105.49.149:8000/lordsexch/card32/',
	  headers: { 
	...
	var getData = await  axios(config);
	  ...
	  socket.emit("FromAPI9",{data:getData.data.data,data1:getData.data.result});
